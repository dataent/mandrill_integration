...
def notify(mandrill_events=None):
	...
	# print dataent.as_json(json.loads(mandrill_events), indent=2)
...
def get_communication(msg):
	...
	if message_id and "@{site}".format(site=dataent.local.site) in message_id:
		communication_name = message_id.strip(" <>").split("@", 1)[0]
		return dataent.get_doc("Communication", communication_name)
	else:
		return None
...
def get_post_url_for_hashing(post_url=None, post_args=None):
	...
	if not post_args: post_args = dataent.local.form_dict
...
def get_webhook_keys():
	...
	def _get_webhook_keys():
		webhook_keys = [d.mandrill_webhook_key for d in dataent.get_all("Email Account",
		...
		if dataent.conf.mandrill_webhook_key:
			webhook_keys.append(dataent.conf.mandrill_webhook_key)

		return webhook_keys

	return dataent.cache().get_value("mandrill_webhook_keys", _get_webhook_keys)

def clear_cache():
	dataent.cache().delete_value("mandrill_webhook_keys")
